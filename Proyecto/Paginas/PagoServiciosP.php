...
<head>
	...
	<title>Pago de servicios públicos</title>

	<link rel="stylesheet"  href="../css/bootstrap.css"> 
	<script>
		function pagar() {
			location.href="PagarServicios.php ? CuentaE=<?=$_GET['CuentaE']?> & id=<?= $_GET['id'] ?>& Cedula=<?=$_GET['Cedula']?>";
		};
	</script>
	<script>
	function Ver(){
		...
		location.href="PagoServiciosP.php ?id=<?= $_GET['id']; ?>& Cedula="+Ced.value+ "& CuentaE="+cuentaE.value;
	};	
	</script>
<body class="container p-5">
	<div class="mt-6 "  align="center" >
		<h1>Pago de servicios públicos</h1>
		...
		<select class="mb-4 mt-4 form-select" id="tipo" aria-label="Default select example">
